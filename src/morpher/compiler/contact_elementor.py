...
def apply_contact_elementor_intent(root: DesignNode) -> DesignNode:
    ...
    Generated contact/amenity pair containers keep their internal item alignment at
    Start. The pair container itself is centered as an item in its owner via Align
    Self, so no child-level alignment overrides are needed. Newsletter submit borders
    keep the authored top/right/left stroke but intentionally omit the bottom edge.
    """
    for node in _walk(root):
        if node.kind == "container" and "::contact-item-" in (node.source_id or ""):
            node.style.counter_axis_align = "min"
