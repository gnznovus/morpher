...
def test_renders_web_sources_in_preferred_order(tmp_path: Path) -> None:
    face = _face(tmp_path)
    ...
    assert "HK Grotesk / 700 / normal / legacy → registry cache hit" in css


def test_renders_fallback_diagnostic(tmp_path: Path) -> None:
    face = _face(tmp_path)
    resolution = FontResolution(
        status="fallback",
    ...
    assert "fallback after registry refresh" in css
    assert "HK Grotesk / 700 / normal / legacy" in css


def test_renders_missing_font_warning() -> None:
    resolution = FontResolution(
        status="missing",
    ...
    assert "MORPHER WARNING: font not found after registry refresh" in css
    assert 'family: "Not Existed Font"' in css
    assert "weight: 700" in css
    assert "style: normal" in css
